...
export type Exists<T> = (args: T[Pk<T>] | Query<RecordOf<T>>) => Promise<boolean>;
...
type FindArgs<T> = "*" | string | Query<RecordOf<T>> | ((x: RecordOf<T>) => boolean);
export type Find<T> = (args: FindArgs<T>) => Promise<RecordOf<T>[]>;
export type Remove<T> = (args: "*" | T[Pk<T>] | Query<RecordOf<T>> | ((x: RecordOf<T>) => boolean)) => Promise<any>;
...
export type Put<T> = ((key: string, value: any, callback: ErrorCallback) => void) & ((key: string, value: any, options: AbstractOptions, callback: ErrorCallback) => void) & ((key: string, value: any, options?: AbstractOptions) => Promise<void>) & ((record: RecordOf<T>) => Promise<void>);
